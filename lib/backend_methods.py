...
def change_photoposition(
    username, password, image_name, latitude, longitude, show_user_content=False
):
    url = BACKEND_URL + "photo/change_photoposition/"
    body = {"image": image_name, "latitude": latitude, "longitude": longitude}
    with requests.session() as s:
        s.auth = (username, password)
        r = s.post(url, body)
        if r.status_code == 201 or r.status_code == 200:
...
def change_description(
    username, password, image_name, description, show_user_content=False
):
...
def add_photos_to_upload_list(
    username, password, image_string, show_user_content=False
):
...
def remove_uploadlist(username, password, date_upload):
    ...
    print(
        response.content
    ) if response.status_code == 201 or response.status_code == 200 else print(
        str(response.status_code) + "wrong with upload"
    )
